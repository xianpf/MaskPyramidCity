...
from grow_model.norm_conv import NormConv2d, MyConv2d, MyNormConv2d
...
class double_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        super(double_conv, self).__init__()
        self.conv = nn.Sequential(
            NormConv2d(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            NormConv2d(out_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class MaskPyramids(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        num_classes = cfg.DATALOADER.NUM_CLASSES
        self.ori_conv = NormConv2d(3, 3, 3, padding=1, bias=False, stretch=False, merge_amp=False)
        self.my_conv = MyNormConv2d(3, 3, 3, padding=1, bias=False, groups=3)
        
        self._init_weight()

    def __init___conv2d_test(self, cfg):
        super(MaskPyramids, self).__init__()
        self.cfg = cfg
        num_classes = cfg.DATALOADER.NUM_CLASSES
        self.ori_conv = nn.Conv2d(3, 8, 3, padding=1, bias=False)
        self.my_conv = MyConv2d(3, 8, 3, padding=1, bias=False)
    ...
    def forward_conv2d_test(self, image, targets=None):
        output_dict = dict()
        N, _, img_h, img_w = image.shape
        ori_conv_out = self.ori_conv(image)
        self.my_conv.weight.data = self.ori_conv.weight.data
        # self.my_conv.bias.data = self.ori_conv.bias.data
        my_conv_out = self.my_conv(image)
        print((my_conv_out - ori_conv_out).abs().max())
        import pdb; pdb.set_trace()
    ...
    def forward(self, image, targets=None):
        ...
        N, _, img_h, img_w = image.shape
        ori_conv_out = self.ori_conv(image)
        self.my_conv.weight.data = self.ori_conv.group_conv.weight.data
        # self.my_conv.bias.data = self.ori_conv.bias.data
        my_conv_out = self.my_conv(image)
        print((my_conv_out - ori_conv_out).abs().max())
        import pdb; pdb.set_trace()
