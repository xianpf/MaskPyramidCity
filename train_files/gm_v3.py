...
from grow_model.norm_conv import ResNormConv2d
...
class double_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(
            ...
            ResNormConv2d(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            ResNormConv2d(out_ch, out_ch, 3, padding=1),
...
class outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = ResNormConv2d(in_ch, out_ch, 1, bias=True)
...
def main():
    ...
    cfg.merge_from_list(['SOLVER.BASE_LR', 1e-2])
    cfg.merge_from_list(['MULTI_RUN', True])
    cfg.merge_from_list(['MODEL.WEIGHT', 'run/gm_v3_sematic_4/model_Epoch_2.pth'])
