...
class double_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        super(double_conv, self).__init__()
        self.conv = nn.Sequential(
            # ResNormConv2d(in_ch, out_ch, 3, padding=1, bias=True),
            MeanResNormConv2d(in_ch, out_ch, 3, padding=1),
            # nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            MeanResNormConv2d(out_ch, out_ch, 3, padding=1),
            # ResNormConv2d(out_ch, out_ch, 3, padding=1, bias=True),
            # nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class up(nn.Module):
    ...
    def _init_weight(self):
        ...
        norm_conv0 = self.conv.conv[0]
        norm_conv0.out_1x1_conv.weight.data[:, :norm_conv0.in_channels] *= 2

    def up_focus(self):
        norm_conv0 = self.conv.conv[0]
        norm_conv0.out_1x1_conv.weight.data[:, :norm_conv0.in_channels] *= 1+1e-2
...
class outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        # self.conv = MeanResNormConv2d(in_ch, out_ch, 1, bias=True)
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(self.train_loader):
            ...
            if epoch < 100 and losses < 1e2:
                ...
                self.model.up4.up_focus()
                self.model.up5.up_focus()
                self.model.up6.up_focus()
                self.model.up7.up_focus()
