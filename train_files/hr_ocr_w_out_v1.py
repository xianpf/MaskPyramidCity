...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.ocr_gather_head = SpatialGather_Module(config.DATALOADER.NUM_CLASSES)
        ...
        self.cls_head = nn.Conv2d(
            ...
            ocr_mid_channels, config.DATALOADER.NUM_CLASSES, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.aux_head = nn.Sequential(
            ...
            nn.Conv2d(last_inp_channels, config.DATALOADER.NUM_CLASSES,
    ...
    def forward(self, image, targets=None):
        ...
        N, C, H, W = feats.shape
        query = feats.view(N,C,-1).permute(0,2,1)
        key = context.view(N,C,19)
        sim_map_w_out = torch.matmul(query, key)
        out = sim_map_w_out.permute(0,2,1).view(N, 19, H, W)
        ...
        # # return out_aux_seg
        sematic_out = F.upsample(out, size=(in_h, in_w), mode='bilinear')
...
# hr_orc_w_out_v1 直接输出W也可以
# hr_orc_v1 测试OCR的语意分割表现； mIoU: 0.6881| Acc: 0.9542| Acc_class: 0.7809| fwIoU: 0.9171| previousBest: 0.6901|
