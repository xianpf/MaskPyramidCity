...
class MaskPyramids(nn.Module):
    ...
    def forward(self, image, targets=None):
        ...
        output_dict['sematic_out'] = sematic_out
        output_dict['panoptic_out'] = panoptic_out
        output_dict['targets'] = targets
        output_dict['ins_pyramids'] =ins_pyramids
        # output_dict['in_dict_watches'] =in_dict_watches
    ...
    def show_image_v8(self, images, output, targets):
        ...
        for i, image in enumerate(images):
            ...
            if not self.cfg.SOLVER.SEMATIC_ONLY:
                ...
                if pred_panoptic_ON:
                    # import pdb; pdb.set_trace()
                    panoptic_out = output['panoptic_out'][i]
                    panoptic_predict_np = panoptic_out[0].max(0)[1].detach().cpu().numpy()
                    pan_cats_predict = [n for n in range(11)]+[pyr.tar_cat for pyr in  parymids[i]]
                    # import pdb; pdb.set_trace()
                    pred_panoptic = self.rend_on_image_v8(images_np, panoptic_predict_np, pan_cats_predict)
                    if pred_stat_ON:
                        cv2.putText(pred_panoptic,'#:{}'.format(len(parymids[i])),(30,30),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255),1)
                        for pyr in parymids[i]:
                            # import pdb; pdb.set_trace()
                            pyr_pos_513 = pyr.get_rel_pos(7)[::-1]
                            cv2.circle(pred_panoptic, pyr_pos_513, 3, (0,0,255), -1)
                    parts_to_show.append(pred_panoptic)
...
def main():
    ...
    cfg.merge_from_list(['MODEL.WEIGHT', 'run/mpc_v10.2_1/model_Epoch_32.pth'])
...
# v10_3: imshow 实际的instance target时与不可分割物体同步展示
