...
# v3: allow show the running image
class MaskPyramids(nn.Module):
    ...
    def forward(self, image, targets=None):
        ...
        output_dict['inst_pyramids'] = inst_pyramids
...
class Trainer(object):
    ...
    def show_image(self, images, output, targets):
        ...
        for i, image in enumerate(images):
            ...
            semt_a_image, insts_a_image = targets['label'][i], targets['instance'][i]
            class_of_inst = [semt_a_image[insts_a_image == j].unique().item() for j in range(len(insts_a_image.unique()))]
            class_names = self.train_loader.dataset.class_names
            insts_a_image_np = insts_a_image.detach().cpu().numpy()
            masked_target, color1 = self.rend_on_image(images_np, insts_a_image_np, class_of_inst)
            # prediction
            # import pdb; pdb.set_trace()
            sematic_a_image = output['sematic_out'][i]
            sematic_a_image_np = sematic_a_image.max(0)[1].detach().cpu().numpy()
            masked_sematic, color2 = self.rend_on_image(images_np, sematic_a_image_np, range(19))
