...
class MaskPyramids(nn.Module):
    ...
    def _init_target(self, img_tensor_shape, device, target=None, run_idx=0):
        ...
        target_levels[1] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[2] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[3] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[4] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[5] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[6] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
        level_shape = ((level_shape[0]+1)//2, (level_shape[1]+1)//2)
        target_levels[7] = F.interpolate(target_mask_pad_to_img.float(), level_shape, mode='bilinear', align_corners=True).type(target_mask_pad_to_img.dtype)
    ...
    def compute_mask(self, level, feature, pyramids, is_init=False):
        ...
        for pyramid in pyramids:
            ...
            feature_gaussian_mask = F.interpolate(pyramid.init_gaussian_mask[None,None,...]\
                .to(feature.device), tuple(feature[0,0].shape), mode='bilinear', align_corners=True)[0,0]
            if is_init:
                ...
                last_mask_logits_up = F.interpolate(last_mask_logits, up_size, mode='bilinear', align_corners=True)
    ...
    def compute_class(self, pyramids, target_levels):
        ...
        for pyramid in pyramids:
            ...
            if self.cfg.DATALOADER.DATASET == 'coco':
                class_logits_5x5 = F.interpolate(class_logits, (5, 5), mode='bilinear', align_corners=True)
    ...
    def forward(self, image, targets=None):
        ...
        if self.cfg.SOLVER.SEMATIC_ONLY:
            output_dict['loss_dict'] = {'class_loss':sematic_loss}
        ...
        for i in range(N):
            ...
            dict_watch['inst_pyramids'] = inst_pyramids
            dict_watch['target_levels'] = target_levels


            # losses_class.append(class_loss)
            # all_pyramids.append(inst_pyramids)
            # all_target_levels.append(target_levels)
            in_dict_watches.append(dict_watch)
...
class Trainer(object):
    ...
    def show_image_l1(self, images, output, targets):
        ...
        inst_target_513_fwd_ON = True
        inst_target_5_fwd_ON = True
        inst_pred_sematic_ON = False
        inst_pred_l0_ON = True
        for i, image in enumerate(images):
            parts_to_show = []
            images_np = image.permute(1,2,0).detach().cpu().numpy()
            images_np = ((images_np - images_np.min()) / (images_np.max() - images_np.min()))[...,::-1]
            ...
            if i % 20 == 0 or i == len(self.train_loader) -1:
                ...
                if self.cfg.SOLVER.SEMATIC_ONLY:
                    self.logger.info(('Ep:{}/{}|Iter:{}/{}|Eta:{}|SematicLoss:{:2.4}|Class:{:2.4}').format(
                        ...
                        eta_string, losses.item(), loss_dict['class_loss'].item()))
                else:
