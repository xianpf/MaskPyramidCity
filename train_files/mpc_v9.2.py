...
class MaskPyramids(nn.Module):
    def __init__(self, cfg):
        ...
        self._sematic_outc_4 = outconv(256, num_classes)
        self._sematic_outc_8 = outconv(256, num_classes)
        self._sematic_outc_16 = outconv(256, num_classes)
        self._sematic_outc_32 = outconv(256, num_classes)
        self._sematic_outc_64 = outconv(256, num_classes)
        self._sematic_outc_128 = outconv(256, num_classes)
        self._sematic_outc_256 = outconv(256, num_classes)
        self._sematic_outc_513 = outconv(256, num_classes)
    ...
    def compute_sematic(self, feature, target):
        # import pdb; pdb.set_trace()
        x_sem_4 = self._sematic_outc_4(feature[7])
        x_logit_8 = self.up1(feature[7], feature[6])
        x_sem_8 = self._sematic_outc_8(feature[6])
        x_logit_16 = self.up2(x_logit_8, feature[5])
        x_sem_16 = self._sematic_outc_16(x_logit_16)
        x_logit_32 = self.up3(x_logit_16, feature[4])
        x_sem_32 = self._sematic_outc_32(x_logit_32)
        x_logit_64 = self.up4(x_logit_32, feature[3])
        x_sem_64 = self._sematic_outc_64(x_logit_64)
        x_logit_128 = self.up5(x_logit_64, feature[2])
        x_sem_128 = self._sematic_outc_128(x_logit_128)
        # x_logit_256 = self.up5(x_logit_128, feature[1])
        # x_sem_256 = self._sematic_outc_256(x_logit_256)
        # x_logit_513 = self.up6(x_logit_256, feature[0])
        x_logit_513 = self.up7(x_logit_128, feature[0])
        x_sem_513 = self._sematic_outc_513(x_logit_513)

        # import pdb; pdb.set_trace()
        sematic_targets_4 = self.prepare_targets(target, mode='sematic', down_size=(4,4))
        sematic_targets_8 = self.prepare_targets(target, mode='sematic', down_size=(8,8))
        sematic_targets_16 = self.prepare_targets(target, mode='sematic', down_size=(16,16))
        sematic_targets_32 = self.prepare_targets(target, mode='sematic', down_size=(32,32))
        sematic_targets_64 = self.prepare_targets(target, mode='sematic', down_size=(64,64))
        sematic_targets_128 = self.prepare_targets(target, mode='sematic', down_size=(128,128))
        # sematic_targets_256 = self.prepare_targets(target, mode='sematic', down_size=(256,256))

        sematic_loss_4 = self.sematic_criteron(x_sem_4, sematic_targets_4)
        sematic_loss_8 = self.sematic_criteron(x_sem_8, sematic_targets_8)
        sematic_loss_16 = self.sematic_criteron(x_sem_16, sematic_targets_16)
        sematic_loss_32 = self.sematic_criteron(x_sem_32, sematic_targets_32)
        sematic_loss_64 = self.sematic_criteron(x_sem_64, sematic_targets_64)
        sematic_loss_128 = self.sematic_criteron(x_sem_128, sematic_targets_128)
        # sematic_loss_256 = self.sematic_criteron(x_sem_256, sematic_targets_256)
        sematic_loss_513 = self.sematic_criteron(x_sem_513, target['label'].long())

        # import pdb; pdb.set_trace()
        sematic_loss = torch.tensor([sematic_loss_4, sematic_loss_8, sematic_loss_16, sematic_loss_32, sematic_loss_64, 
            sematic_loss_128, sematic_loss_513], requires_grad=True)
        # sematic_loss = torch.tensor([sematic_loss_5, sematic_loss_9, sematic_loss_17, sematic_loss_33, 
        #     sematic_loss_65, sematic_loss_129, sematic_loss_513])

        return sematic_loss, [x_sem_4, x_sem_8, x_sem_16, x_sem_32, x_sem_64, x_sem_128, x_sem_513]
    ...
    def forward(self, image, targets=None):
        ...
        # import pdb; pdb.set_trace()
        sematic_loss, sematic_out = self.compute_sematic(down_features, targets)
